...
export const ProductCard: React.FC<ProductCardProps> = ({ product, isSellerView = false, onEdit, onQuickView, onUpdate }) => {
    ...
    const { translations, comparisonList, toggleCompare } = context;
        ...
        <div onClick={() => onQuickView?.(product)} className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden group flex flex-col relative cursor-pointer">
            {!isSellerView && (
                 <button
                    ...
                    className={`absolute top-2 right-2 z-10 p-2 rounded-full transition-all duration-200 ease-in-out ${
                        isComparing 
            ...
            <div className="relative w-full aspect-square overflow-hidden">
                 ...
                 {!isSellerView && (
                     <div className="absolute inset-0 bg-black/40 hidden md:flex items-center justify-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        <button 
